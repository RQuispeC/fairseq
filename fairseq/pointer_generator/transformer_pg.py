...
class TransformerPointerGeneratorEncoder(TransformerEncoder):
    ...
    def forward_embedding(
        ...
        if token_embedding is None:
            token_embedding = self.embed_tokens(src_tokens.transpose(1, 0)).transpose(1, 0)
        x = embed = self.embed_scale * token_embedding
        if self.embed_positions is not None:
            x = embed + self.embed_positions(src_tokens)
        if self.layernorm_embedding is not None:
            x = self.layernorm_embedding(x.transpose(1, 0)).transpose(1, 0)
...
class TransformerPointerGeneratorDecoder(TransformerDecoder):
    ...
    def forward(
        ...
        if not features_only:
            ...
            prev_output_embed = self.embed_tokens(prev_output_tokens.transpose(1, 0)).transpose(1, 0)
            prev_output_embed *= self.embed_scale
            predictors = torch.cat((prev_output_embed, x), 2)
            p_gens = self.project_p_gens(predictors.transpose(1, 0)).transpose(1, 0)
    ...
    def extract_features_scriptable(
        ...
        # embed tokens and positions
        x = self.embed_scale * self.embed_tokens(prev_output_tokens.transpose(1, 0)).transpose(1, 0)
    ...
    def output_layer(
        ...
        if self.adaptive_softmax is None:
            logits = self.output_projection(features.transpose(1, 0)).transpose(1, 0)
        else:
            logits = features
